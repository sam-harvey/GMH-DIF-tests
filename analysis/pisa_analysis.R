...
pisa_analysis = function(mat_in,
                         file_path_out = "output/pisa_results.RData",
                         alpha=0.05,
                         within_group=F,
                         zeros=F,
                         m=35,
  ...
  dat1 <- mat_in[, 3:(m + 2)]
  dat1 <- cbind(as.integer(mat_in[, "group"]), as.integer(as.factor(mat_in[, "Gender"])), dat1)
  ...
  clusterExport(sim_cluster, ls()[ls() != "sim_cluster"], envir = environment())
  ...
  bootstramp_samples <- parLapply(
    ...
    1:BT,
    function(x) {
      y.boot <- obtain.bootstrap.samples(y.sim, within.group = within_group, K = K)
