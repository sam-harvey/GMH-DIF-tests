...
df_aus_students = df_scored_cogs %>% 
  ...
  inner_join(df_student %>% 
               dplyr::select(StIDStd = STIDSTD, CNT, ST04Q01)) %>% 
  dplyr::select(StIDStd, ST04Q01, starts_with('P')) 

df_aus_students = df_aus_students %>% 
  inner_join(df_student_data_dict %>% 
               filter(VARIABLE_NAME=="ST04Q01") %>% 
               mutate(VALUE = as.numeric(VALUE)) %>% 
               dplyr::select(VALUE, Gender = VALUE_LABEL), by = c('ST04Q01' = 'VALUE')) %>% 
  dplyr::select(StIDStd, Gender, everything())
...
df_aus_students = df_aus_students %>% 
  dplyr::select(StIDStd, Gender, test_cols) %>% 
