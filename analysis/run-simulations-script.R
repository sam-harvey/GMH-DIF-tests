...
walk(
  1:nrow(df_dist_variables),
     possibly(function(x){
       ...
       file.name.gen <- glue("Sim_GenDif_m{ifelse(experiment_params$m > experiment_params$m0, experiment_params$m0, experiment_params$m)}_K{experiment_params$K}_Gamma{experiment_params$gamma}_OR{experiment_params$OR}.RData")
