...
construct.tests <- function(L, VarL, CovL, alpha = 0.05, show = TRUE) {
  ...
  if (!is.positive.definite(CovL)) {
    if (show) {
      cat("Covariance matrix is not pos definite")
    }
    ...
    if (show) {
      cat("eigenvalues of original matrix CovL\n")
    ...
    if (show) {
      cat("eigenvalues of new matrix Corr\n")
    ...
    if (show) {
      cat("Provided Covariance matrix is pos definite")
    }
  ...
  return(list(
    pvalues = pvalues, CovL = CovL, CovLold = CovLold, W = W, Wind = Wind,
