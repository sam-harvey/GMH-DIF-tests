generate_pisa_output_tables = function(pisa_results_file = 'output/pisa_results.RData',
                                       pisa_responses_file = 'data/pisa_aus_science_book_1_responses.csv',
                                       output_prefix = 'normal',
                                       pairwise_question_index = NULL){
  
  load(pisa_results_file)
  df_pisa_aus = read_csv(pisa_responses_file)
  ...
  df_output_table %>% 
    write_csv(glue('output/tables/{output_prefix}-pisa-estimate-table.csv'))
  
  df_output_table %>% 
    mutate_if(is.numeric, ~round(., 4)) %>% 
    write_csv(glue('output/tables/{output_prefix}-pisa-estimate-table-rounded.csv'))
  ...
  if(is.null(pairwise_question_index)){
    question_index = which(tests$pvalues < 0.05)
  } else{
    question_index = pairwise_question_index
  }
  ...
  cv_tbl %>% 
    write_csv(glue('output/tables/{output_prefix}-dif-pairwise-dif-example.csv'))
  ...
  tbl_output %>% 
    write_csv(glue('output/tables/{output_prefix}-or-pair-output.csv'))
  ...
  data.frame(Method = c('Asym', 'Boot'),
    ...
    write_csv(glue('output/tables/{output_prefix}-output-table-pisa-W-comparison.csv'))
