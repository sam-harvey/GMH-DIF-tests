...
size<-dim(X);
#if(length(size)==2){
if(method==1){h<-matrix(1,1,size[2]);
for(i in 1:size[1]){
h<-h*X[i,];}
}else{h<-matrix(1,size[1],1);
for(i in 1:size[2]){
h<-h*X[,i];}
}
#}else{
...
if(is.vector(x)){
lx<-length(x);x<-matrix(x,lx,1);
}
if(is.vector(y)){
ly<-length(y);y<-matrix(y,ly,1);
}
diff<-y-x;ones<-matrix(1,lx,1);ones[diff<0]<-0;
hilf<-x+(y-x)*ones;
hilf
}
...
for(i in 1:c){
 hilf<-dec-2^(c-i);
 f<-(hilf>=0)
 if(f){dec<-hilf;bin[i]<-1}else{dec<-dec}
}#end for
bin
}#end function
...
for(i in 1:LODDS){
      ...
      s1<-min(margp[row],margp[col]);s2<-max(0,margp[row]+margp[col]-1);

      cond<-(hilf<=s1 & hilf>=s2);
      #print(cond);print(hilf);print(s1);print(s2)
      if(cond[1]){pairpp<-rbind(pairpp,hilf[1])}
      else{
      if(cond[2]){pairpp<-rbind(pairpp,hilf[2])}}
      if(!max(cond)){print("ERROR computation of roots")}
}#for i=1:LODDS
...
if(pair){pairpp<-pairwise(odds,margp)}else{pairpp<-odds};
#print(pairpp)
B<-matrix(0,c2,c)
for(i in 1:c2){
...
for(order in c(1,2,3)){   
   ...
   if(order==3){p<-p*1/sum(p);   
   diff<-max(diff,norm(p-p0));}
   
  }#for(order in 1:3){  
  if(iterations%%1e2==0){cat(" ",diff,iterations,"\n"); }
  } #while(norm(p-p0))>1e-6)

print("Number iterations");print(iterations)
p_joint<-p;
#print(p)
}else{
...
cmarg<-cbind(cmarg,margp);print(cmarg)
...
cpair<-cbind(cpair,pairpp);print(cpair)
...
if(r>2){
  sK<-cumsum(K);
 for(i in 2:(r-1)){
  NP<-NP*choose(n-sK[i-1],K[i]);}#end for;
}#end if;
NP
...
LODDS<-c*(c-1)/2;
index<-indexfct(c);
errors<-0;marg.ind<-NULL;pair.ind<-NULL
if(c>2){
...
if(hilf>1 | hilf<0){cat("ERROR: Pair+Marg do not aggree:",hilf," \n");
errors<-errors+1;marg.ind<-rbind(marg.ind,ind);
...
if(c>2){Z1<-get_Z_second(c-1)}else{Z1<-h; return(Z1)};
Z1<-cbind(Z1,Z1);
...
X11[UT]<-h11;X10[UT]<-h10;X01[UT]<-h01;X00[UT]<-h00;
...
X[i2,i1]<-X[i2,i1]+0.5;X1[i2,i1]<-X1[i2,i1]+0.5;
#n[i1]<-n[i1]+1;
...
#assign values from matrices to uppertri arrays
X11[UT]<-h11;X10[UT]<-h10;X01[UT]<-h01;X00[UT]<-h00;
X11<-aperm(X11,c(2,1,3));X10<-aperm(X10,c(2,1,3));
X01<-aperm(X01,c(2,1,3));X00<-aperm(X00,c(2,1,3));
...
#print(n0)
 ...
 Cxy<-Cxy+cxy/n0;Cyx<-Cyx+cyx/n0; #but here
 }
 if(n1>1){ 
 ctxy<-(cxy-h10[i])/n1 ;ctyx<-(cyx-h01[i])/n1; 
 Ctxy<-Ctxy+ctxy;Ctyx<-Ctyx+ctyx;
 }
...
z1<-Cxy^2;z2<-Cxy*Cyx;z3<-Cyx^2
v1<-Ctxy^2;v2<-Ctxy*Ctyx;v3<-Ctyx^2
...
comb3<-combn(c,3);
n.comb3<- dim(comb3)[2];
}else{comb3<-NULL;n.comb3<-0;}
if(c>3){
comb4<-combn(c,4);
n.comb4<-dim(comb4)[2];
}else{comb4<-NULL;n.comb4<-0;}
...
set3<-1:c;
 ...
 for(x in set3){
  ...
  VA12[x,z,y]<-VA12[x,y,z];VA21[x,z,y]<-VA21[x,y,z];#OK
  VB12[x,z,y]<-VB12[x,y,z];VB21[x,z,y]<-VB21[x,y,z];#OK
...
for(i in 1:c){
 for(j in i:c){
  if(i==j){Uplus[i,i]<-sum(U[i,,]);Oplus[i,i]<-sum(O[i,,]);#OK
  }else{
   Lbar[i,j]<-(Lplus[i]-Lplus[j])/c;Lbar[j,i]<- -Lbar[i,j];#OK
   if(VAR){
...
for(i in 1:c){
 for(j in i:c){
  if(i==j){Uplus[i,i]<-sum(U[i,,]);#Oplus[i,i]<-sum(O[i,,]);
  }else{
   Lbar[i,j]<-(Lplus[i]-Lplus[j])/c;Lbar[j,i]<-Lbar[i,j];
   #if(VAR){
...
L.vector<-function(L){
c<-dim(L)[1];
if(is.null(c)){return(L)}
if(dim(L)[2]==1){return(c(t(L)))}
ind<-indexfct(c);
l.ind<-dim(ind)[2];
L.vec<-matrix(0,1,l.ind)
L.names<-matrix(0,1,l.ind)
for(i in 1:l.ind){L.vec[i]<-L[ind[1,i],ind[2,i]];L.names[i]<-paste(ind[1,i],ind[2,i])}
...
Z<-cbind(matrix(0,cNkK,c),kronecker(matrix(1,Nk,1),diag(K*c)));
 ...
 for(i in 1:NkK){
   ...
   if(data0[i,3]==1){Z[index,1:c]<-diag(c)};
 }#end for i
}
if(r==1){
if(K>1){
...
for(i in 1:n){
 for(j in i:n){
i1<-ind[,i][1];j1<-ind[,j][1];
i2<-ind[,i][2];j2<-ind[,j][2];

#V2_{i1,i2} or equivalently i==j
if(i1==j1 & i2==j2){Cov[i,j]<- V[i1,i2,i2]};
#V3_{i1,i2,j2}
if(i1==j1 & i2!=j2){Cov[i,j]<- V[i1,i2,j2]};
#U_{i1,i2,j2}
if(i1!=j1 & i2==j2){Cov[i,j]<-V[i2,i1,j1]};
#if i1!=j1 and i2!=j2
if(i1!=j1 & i2!=j2){
#i1==j2 which means i2!=j1
if(i1==j2){Cov[i,j]<-    -V[i1,j1,i2]};
#i2==j1 which means i1!=j2
if(i2==j1){Cov[i,j]<-    -V[j1,j2,i1]};
#now i1\neq j2 and i2 \neq j1
if(i2!=j1 & i1!=j2){
...
for(i in 1:l.ind){
 for(j in i:l.ind){
i1<-ind[1,i];j1<-ind[1,j];
i2<-ind[2,i];j2<-ind[2,j];
...
#U_{i1,i2,j2}
if(i1==j1 & i2!=j2){U[i1,j2,i2]<-U[i1,i2,j2]<-Cov.BT[i,j]};
#U_{i1,i2,j2}
if(i1!=j1 & i2==j2){U[i2,i1,j1]<-U[i2,j1,i1]<-Cov.BT[i,j]};
#if i1!=j1 and i2!=j2
if(i1!=j1 & i2!=j2){
#i1==j2 which means i2!=j1
if(i1==j2){U[i1,j1,i2]<-U[i1,i2,j1]<- -Cov.BT[i,j]};
#i2==j1 which means i1!=j2
if(i2==j1){U[i2,j2,i1]<-U[i2,i1,j2]<- -Cov.BT[i,j]};
#now i1\neq j2 and i2 \neq j1
if(i2!=j1 & i1!=j2){
...
if(is.null(L)){L<-rep(0,1,l.ind)}
  ...
  Lnew<-matrix(0,c,c);Lnew[lower.tri(Lnew)]<-L;Lnew<-t(Lnew)
  L1<-Lnew+t(-Lnew)
...
l<-dim(index)[2];
if(length(true.stat)==1){true.stat<-matrix(true.stat,l,1)}
...
colnames(m0)<-names;
if(VAR){
colnames(var0)<-names;return(list(mse=mse0,m=m0,var=var0))
}else{
...
# print(C1ab);print(C2ab);print(C1ba);print(C2ba)
 ...
 if(C1ba==0 | C2ba==0 | C1ab==0 | C2ab==0){   return(NULL) }else{
